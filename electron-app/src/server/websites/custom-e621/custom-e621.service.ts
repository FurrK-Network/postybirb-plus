...
import { getBaseE621Url } from '../../../app/utils';
import { LoginResponse } from '../interfaces/login-response.interface';
import { ScalingOptions } from '../interfaces/scaling-options.interface';
import { Website } from '../website.base';

@Injectable()
export class CustomE621 extends Website {
  readonly BASE_URL: string = getBaseE621Url();
  ...
  preparseDescription(text: string) {
    return UsernameParser.replaceText(text, 'e6c', '@$1').replace(
      /<a(.*?)href="(.*?)"(.*?)>(.*?)<\/a>/gi,
      '"$4":$2',
    );
  }
  ...
  async postFileSubmission(
    ...
    if (post.body.success && post.body.location) {
      return this.createPostResponse({ source: `${this.BASE_URL}${post.body.location}` });
    }
